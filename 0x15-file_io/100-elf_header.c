 ...
 * @e_ident: A pointer to an array with the ELF magic number.
 */
void print_magic(unsigned char *e_ident)
{
	int index;
...
/**
 * print_class - Prints the ELF header's class.
...
/**
 * print_data - Prints the ELF header data.
 ...
 *
 * Description: exit code 98 if file cant be closed.
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
 ...
 * Return: 0 on success.
 *
 * Description exit code 98: if is not an ELF File or the function fails.
 */
